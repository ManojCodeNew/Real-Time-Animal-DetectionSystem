...
class StatisticalPredictor:
    ...
    def predict_next_intrusion(self, species_name: str):
        ...
        Uses pure Python to avoid C-extension build issues.
        """
        ...
        entry_hours = [e.entry_time.hour for e in species_events]
        ...
        peak_hour_events = [e for e in species_events if e.entry_time.hour == peak_hour]
        ...
        return {
            ...
            "historical_data_points_used": len(species_events),
            "peak_activity_window": f"{peak_hour:02d}:00 - {peak_hour:02d}:59"
        }
        
    def get_analytics_summary(self):
        events = crud.get_recent_events(self.db, limit=1000)
        if not events:
            return {"total_intrusions": 0}
